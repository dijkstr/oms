...
<head>
	...
	<script type="text/javascript">
		...
		// 导出预付款查询列表 
		function orderAdvPaymentExport(){
			var params = $("#searchForm").serialize();
			window.open("${ctx}/charge/bill/export/download?templatekey=orderAdvPaymentExport&" + params);
		}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/charge/report/orderAdvPayment/">预付款查询列表</a></li>
		<shiro:hasPermission name="charge:report:orderAdvPayment:edit"><li><a href="${ctx}/charge/report/orderAdvPayment/form">预付款查询添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="searchForm" action="${ctx}/charge/report/orderAdvPayment/" method="post" class="breadcrumb form-search" style="height:150px;">
	...
	<div style="padding-bottom: 10px;">
		<input id="export" class="btn btn-primary" type="button" value="导出预付款查询" onclick="orderAdvPaymentExport()"/>
	</div>
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>预付金额</th>
		<tbody>
		<c:forEach items="${page.list}" var="orderAdvPayment">
			<tr>
				<td>${fns:getOfficeNameByCode(orderAdvPayment.office_id)}</td>
				<td>${orderAdvPayment.customer_id}</td>
				<td>${orderAdvPayment.customer_name}</td>
				<td>${orderAdvPayment.contract_id}</td>
				<td>${orderAdvPayment.product_ids}</td>
				<td>${orderAdvPayment.product_names}</td>
				<td>${orderAdvPayment.payment_type}</td>
				<td>${orderAdvPayment.display_date}</td>
				<td><fmt:formatNumber value="${orderAdvPayment.amount}" pattern="#,###.####"/></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
