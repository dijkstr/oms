...
import com.hundsun.boss.modules.charge.service.receipt.ExportReceiveService;
 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/charge/account/receive/export")
public class ExportReceiveController extends ApplicationObjectSupport{
    @Autowired
    private SysConfigService sysConfigService;
    @Autowired
    private ExportReceiveService exportReceiveService;
    
    
    @SuppressWarnings("deprecation")
    @RequestMapping(value = "/download")
    public @ResponseBody Map<String, String> download(String templatekey, ChargeReceiptForm downloadForm, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            try {
                ...
                if (CommonUtil.isNullorEmpty(templatekey)) {
                    String[] billIds = downloadForm.getReceiveid().split(",");
                    for (String billId : billIds) {
                        
                        ReportConfig reportConfig = sysConfigService.getReportCongfig(templatekey);
                        downloadForm.setReceiveid(billId);
                        exportReceiveService.generateReport(zipFileName, reportConfig, downloadForm, filePath);
                    }
                } else {
                    ReportConfig reportConfig = sysConfigService.getReportCongfig(templatekey);
                    exportReceiveService.generateReport(zipFileName, reportConfig, downloadForm, filePath);
